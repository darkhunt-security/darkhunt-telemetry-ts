...
export class Span {
  ...
  constructor(args: SpanCtorArgs) {
    ...
    this.otelSpan = this.tracer.startSpan(args.name, undefined, parentCtx);
    ...
    if (opts.metadata) this.otelSpan.setAttribute(ATTR.METADATA, JSON.stringify(opts.metadata));
    if (opts.level) this.otelSpan.setAttribute(ATTR.OBSERVATION_LEVEL, opts.level);
    if (opts.statusMessage) this.otelSpan.setAttribute(ATTR.STATUS_MESSAGE, opts.statusMessage);
  ...
  update(options: SpanUpdateOptions): this {
    ...
    if (options.level === 'ERROR') {
      this.otelSpan.setStatus({ code: SpanStatusCode.ERROR, message: options.statusMessage });
    } else {
